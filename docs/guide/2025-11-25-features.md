...
Tasks are durable state machines for long-running operations. Instead of returning a result synchronously, the server creates a Task; the client polls (`tasks/get`) or awaits (`tasks/result`) until terminal status.
